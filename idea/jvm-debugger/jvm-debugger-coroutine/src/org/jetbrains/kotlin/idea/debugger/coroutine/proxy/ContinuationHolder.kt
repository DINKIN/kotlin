    ...
    fun extractCoroutineInfoData(continuation: ObjectReference, context: DefaultExecutionContext): CoroutineInfoData? {
        try {
            ...
            for (frame in continuationStack.coroutineStack) {
                val coroutineStackFrame = createStackFrameItem(frame, context)
                if (coroutineStackFrame != null)
                    consumer.add(coroutineStackFrame)
            }
            val lastRestoredFrame = continuationStack.coroutineStack.last()
            return findCoroutineInformation(lastRestoredFrame.baseContinuationImpl.coroutineOwner, context, consumer)
        } catch (e: Exception) {
